...
function createDataDef(names, schema, isInputObjectType, data, links, oas) {
    ...
    if (index !== -1) {
        ...
        if (oneOfConsolidated) {
            ...
            if (oneOfConsolidated.consolidatedTargetGraphQLTypes.some(// Some because not all member schemas have a type, could just be a required field, for example
                        ...
                        if (subSchema.type === 'object') {
                            ...
                            utils_1.handleWarning({
                                ...
                                message: `Union member type '${JSON.stringify(subSchema)}' in ` +
                                    `union type '${JSON.stringify(consolidatedSchema)}' is not an object ` +
